...
param(
	$SiteCode = "P01",
	$DPGroup = "P01 DP",
	$DomainNetbiosName = "INTRA",    
	$UserOUPath = "OU=USR-Verwaltung,OU=PHBern,DC=intra,DC=phbern,DC=ch", 
    $DeviceOUPath = "OU=Clients,OU=PHBern,DC=intra,DC=phbern,DC=ch",    
	$ADGroupNamePrefix = "APP-",
	[bool]$CreateApplicationFolder = $true,
	$ApplicationFolderName = "Development",
	$CollectionFolderName = "Applications",   
	$DeviceLimitingCollection = "All Workstations", 
    ...
    $PADTInstallProgram = "Deploy-Application.ps1 -DeploymentType `"Install`" -DeployMode `"Silent`"",
    $PADTUninstallProgram = "Deploy-Application.ps1 -DeploymentType `"Uninstall`" -DeployMode `"Silent`""
)
...
')
 ...
 #----------------- put into mba
$cpath = split-path $SCRIPT:MyInvocation.MyCommand.Path -parent
$afolder =$cpath.Split("\") 

if ($afolder.Count -eq 7)
    {
    $vendor = $afolder[3]
    $Software = $afolder[4]
    $Version = $afolder[5]
    $internversion = $afolder[6]
    }
if ($Version -eq (get-item -Path $cpath | select parent).parent.name)
    {
    $ApplName = $vendor + " " + $Software + " " + $Version 
    $Applpath = "\\s-prd-ps-001\j$\Sources\Applications\$vendor\$Software\$Version\$internversion"
    }
# ------------------put into mba
...
$LabelAuthor.Text = "Author: Michael Baltes"
#~~< LabelBlog >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
function Load-Form
{
	...
	$ProgressBar1.Step = 1
    #------mba added---------------
    $TextBoxAppName.Text = $ApplName + " EN " + $internversion
    $TextBoxSourcePath.Text = $Applpath
    $TextBoxPublisher.Text = $vendor
...
function Validate-Form
{
    ...
    $ADGroupName = "" # changed by mba from $ADGroupName = $TextBoxADGroup.Text
...
function ButtonCreateClick
{
    ...
    #PHBern mba Collection
    $CollectionName = "Install - " + $TextBoxCollection.Text
    $CollectionNameUnInst = "UnInstall - " + $TextBoxCollection.Text
	...
	if ($OkToProceed)
	{
		...
		if ($CheckBoxCreateCollection.Checked)
		{
			...
			if ($RadioButtonDevice.Checked)
			{
                ...
                # PHB Collection for Uninstall
                $AppCollectionUnInst = New-CMDeviceCollection -Name $CollectionNameUninst -LimitingCollectionName $DeviceLimitingCollection -RefreshType Periodic -RefreshSchedule $Schedule
				Write-Host "Created device collection " -NoNewline; Write-Host $CollectionNameUninst -ForegroundColor Green
				$ProgressBar1.PerformStep()
				...
				If ($CollectionFolderName -ne "")
				{
                    ...
                    #PHB Collection to move Uninstall
					Move-CMObject -FolderPath $CollectionFolderPath -InputObject $AppCollectionUnInst
					Write-Host "Moved collection " -NoNewline; Write-Host $CollectionNameUnInst -ForegroundColor Green -NoNewline; Write-Host " to folder " -NoNewline; Write-Host $CollectionFolderName -ForegroundColor Green
					$ProgressBar1.PerformStep()
				}
			}
		...
		if ($TextBoxMSIPackage.Text.Length -gt 0)
		{
			...
			{
				New-CMApplicationDeployment -CollectionName $AppCollection.Name -Name $ApplicationName -DeployPurpose $DeployPurpose
                ...
                # Added by mba for uninstall Deployment
                New-CMApplicationDeployment -CollectionName $AppCollectionUnInst.Name -Name $ApplicationName -DeployPurpose $DeployPurpose -DeployAction Uninstall
				Write-Host "Created " -NoNewline; Write-Host $DeployPurpose -ForegroundColor Green -NoNewline; Write-Host " deployment for " -NoNewline; Write-Host $ApplicationName -ForegroundColor Green -NoNewline; Write-Host " to collection " -NoNewline; Write-Host $AppCollectionUnInst -ForegroundColor Green
				$ProgressBar1.PerformStep()
			}
		}
        ...
        if ($TextBoxMSIPackage.Text.Length -eq 0 -and $TextBoxAppVPackage.Text.Length -eq 0 -and ($TextBoxInstallProgram.Text.Length -gt 0 -and $TextBoxSourcePath.Text.Length -gt 0))
        {
			...
			if ($CheckBoxCreateDeployment.Checked)
			{
                ...
                # Added by mba for uninstall Deployment
                New-CMApplicationDeployment -CollectionName $AppCollectionUnInst.Name -Name $ApplicationName -DeployPurpose $DeployPurpose -DeployAction Uninstall
				Write-Host "Created " -NoNewline; Write-Host $DeployPurpose -ForegroundColor Green -NoNewline; Write-Host " deployment for Uninstall " -NoNewline; Write-Host $ApplicationName -ForegroundColor Green -NoNewline; Write-Host " to collection " -NoNewline; Write-Host $CollectionNameUnInst -ForegroundColor Green
				$ProgressBar1.PerformStep()
			}
