...
param(
	$SiteCode = "PHB",
	$DPGroup = "PHB DP",
	$DomainNetbiosName = "SERVICES",    
	$UserOUPath = "OU=Users,OU=Verwaltung,DC=VERWALTUNG,DC=PHBERN,DC=LOCAL", 
    $DeviceOUPath = "OU=Clients,OU=Verwaltung,DC=VERWALTUNG,DC=PHBERN,DC=LOCAL",    
	$ADGroupNamePrefix = "APP-",
	[bool]$CreateApplicationFolder = $true,
	$ApplicationFolderName = "Entwicklung",
	...
	if ($OkToProceed)
	{
		...
		if ($CheckBoxCreateCollection.Checked)
		{
			...
			if ($RadioButtonDevice.Checked)
			{
                ...
                #PHBERN
                $AppCollectionUninst = New-CMDeviceCollection -Name $CollectionName -LimitingCollectionName $DeviceLimitingCollection -RefreshType Both -RefreshSchedule $Schedule
				Write-Host "Created device collection " -NoNewline; Write-Host $CollectionName -ForegroundColor Green
				$ProgressBar1.PerformStep()
...
function AppName-Changed
{
	if ($TextBoxAppName.Text -ne "")
	{
        ...
        $TextBoxCollection.Text = "Install - " + $TextBoxAppName.Text
        # PHBERN Anpassung-------------------------------------------------
        $TextBoxCollectionUninstall = "UnInstall - " + $TextBoxAppName.Text
        # PHBERN Anpassung-------------------------------------------------
        $ErrorProviderAppName.Clear()
